...
$sql = 'INSERT INTO import (date, type, text) VALUES (\''.date('Y-m-d H:i:s', time()).'\', \'vacant_land\', \'Save data : begin\')';
$connexion->getConnexion()->query($sql);
...
function recupFichier()
{
	...
	$rets->Disconnect();
	$connexion = new CConnexion(BD_NAME, BD_LOGIN, BD_PASSWORD);
	$sql = 'INSERT INTO import (date, type, text) VALUES (\''.date('Y-m-d H:i:s', time()).'\', \'vacant_land\', \'File creat : done\')';
	$connexion->getConnexion()->query($sql);
...
{
	$connexion = new CConnexion(BD_NAME, BD_LOGIN, BD_PASSWORD);
	$sql = 'UPDATE data SET flag_actif=0 WHERE type=\'vacant_land\' ';
	$connexion->getConnexion()->query($sql);
	$phrets = new phRETS;
	$rets_login_url = "http://mfr.rets.interealty.com/Login.asmx/Login";
	$rets_username = "RETS689";
...
//					   		break;
	...
	$sql = 'UPDATE data SET actif=0 WHERE flag_actif=0 AND type=\'vacant_land\' ';
	$connexion->getConnexion()->query($sql);
}
?>