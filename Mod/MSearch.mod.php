...
class MSearch
{
	...
	public function searchForm($type_style, $city, $beds, $bathroom, $option, $price, $page)
	{
		...
		$req .= ' ORDER BY price DESC LIMIT '.$n.', 6';
	...
	public function getAllDatasCsv($id_property, $type_property, $file)
	{
		...
		$tabRows =  array();
		$req = 'SELECT json_data FROM data WHERE id= \''.$id_property.'\' AND type = \''.$type_property.'\' LIMIT 1';
		$res = $this->dbData->getConnexion()->query($req); //on récupère une connexion
		if($res !== false)
			$tabRows = $res->fetchAll(PDO::FETCH_ASSOC);

		if(count($tabRows) > 0 && isset($tabRows[0]['json_data']) && $tabRows[0]['json_data'] != '')
		{
			return json_decode($tabRows[0]['json_data'], true);
		}
		else
		{
